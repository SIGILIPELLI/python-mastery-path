"""
Muxes each scene's narration/dialogue audio into its rendered .webm clip,
transcodes to h264/aac mp4, and concatenates all scenes into the final
video. Same approach as the coding-tutorial pipeline's assemble/assemble.py.
"""
import subprocess
...
def assemble_video(clip_paths: list[Path], audio_paths: list[Path], out_path: Path, vertical: bool) -> Path:
    ...
    for idx, (clip, audio) in enumerate(zip(clip_paths, audio_paths)):
        ...
        try:
            ...
            raise RuntimeError(f"ffmpeg mux failed on scene {idx}: {e.stderr}") from e
